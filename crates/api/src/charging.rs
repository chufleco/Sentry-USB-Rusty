...
/// Auto-applied home tag label.
const HOME_TAG: &str = "Home";

/// Reserved label for the derived "Fast charging" filter (filter-only, never a
/// stored tag) — kept out of the tag store like `HOME_TAG`.
const FAST_TAG: &str = "Fast charging";
...
/// Drop the reserved, derived "Home" tag (any casing) from user-supplied tags
/// so it can never be stored (or synced to the cloud) as an editable tag.
fn strip_reserved_tags(tags: Vec<String>) -> Vec<String> {
    tags.into_iter()
        .filter(|t| !t.eq_ignore_ascii_case(HOME_TAG) && !t.eq_ignore_ascii_case(FAST_TAG))
        .collect()
}
...
fn is_home_charge(lat: Option<f64>, lon: Option<f64>, home: Option<(f64, f64, f64)>) -> bool {
    match (lat, lon, home) {
        (Some(la), Some(lo), Some((hla, hlo, r))) => distance_m(la, lo, hla, hlo) <= r,
        _ => false,
    }
}
...
pub async fn set_charge_tags(
    ...
    let store = state.drives.store.clone();
    // "Home" is a reserved, derived tag — strip it before storing (see
    // strip_reserved_tags): keeps it out of the store and the cloud envelope.
